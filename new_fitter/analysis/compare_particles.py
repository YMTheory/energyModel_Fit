...
def main():
    ...
    nonl_elec = np.array(nonl_elec)



    ax0.plot(Etrue_posi, best4, "-.",  color="royalblue", lw=2, zorder=3, label=r"$e^+$ with $1\sigma$ errorbars $\times 5$")
    ax0.fill_between(Etrue_posi, best4-5*(best4-ymin4), best4+5*(ymax4-best4), alpha=1.0, color="lightskyblue")
    ax0.plot(Etrue, nonl_elec, "-", lw=2, color="coral", label=r"$e^-$ with $1\sigma$ errorbars $\times 5$")
    ax0.fill_between(Etrue, nonl_elec - 5*(nonl_elec-ymin6), nonl_elec + 5*(ymax6-nonl_elec), alpha=1.0, color="bisque")
    ax0.errorbar(gamE, gammu/global_es/gamE, yerr=10*gammuerr/global_es/gamE,fmt="o--", lw=2, ms=5, color="seagreen", label=r"Single $\gamma$ with $1\sigma$ errorbars $\times$ 10")
    ax0.set_xlabel(r"$E_{dep}$[MeV]", fontsize=17)
    ax0.set_ylabel(r"$E_{vis}/E_{dep}$", fontsize=17)
    ax0.set_ylim(0.86, 1.06)
    ...
    ax1.plot(npe_posi/global_es, best_posi1/npe_posi, "-.", lw=2, color="blue", label=r"$e^+$ with $1\sigma$ errorbars", zorder = 2)
    ax1.fill_between(npe_posi/global_es, ymin1/npe_posi, ymax1/npe_posi, color="lightskyblue")
    ax1.errorbar(gammu/global_es, gamsigma/gammu, yerr=5*np.sqrt(gammuerr**2*gamsigma**2/gammu**4 + gamsigmaerr**2/gammu**2), fmt="o--", lw=2, ms=5, color="seagreen", label=r"Single $\gamma$ with $1\sigma$ errorbars $\times$ 5")
    ax1.plot(resE_elec/global_es, res_elec/(resE_elec), color="coral", lw=2, label=r"$e^-$ with $1\sigma$ errorbars")
    ax1.fill_between(resE_elec/global_es, ymin7/resE_elec, ymax7/resE_elec, alpha=0.6, color="bisque")
    ax1.set_xlabel(r"$E_{vis}$[MeV]", fontsize=17)
    ax1.set_ylabel(r"$\sigma_E/E_{vis}$", fontsize=17)
    ax1.legend(prop={"size":15})
    ax1.tick_params(axis='both', which='major', labelsize=16)
    ax1.set_ylim(0.010, 0.044)
    ax1.grid(True)
